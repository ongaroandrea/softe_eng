...
class Authenticator {
    ...
    /**
     * Checks if a user with the username specified in the parameter has any of the specified roles.
     * 
     * @param username - The username of the user to check if it has a specific role.
     * @param roles - The roles to check if the user has.
     * @returns true if the user with username is any of the specified role.
     */
    hasRole(username: string, roles: [string]) {
        const udao = new UserDAO();

        return udao.getUserByUsername(username)
                   .then(user => roles.includes(user.role))
                   .catch(_ => false);
    }
    ...
    /**
     * Middleware function to check if the user is an admin or a manager.
     * 
     * @param req - The request object.
     * @param res - The response object.
     * @param next - The next middleware function.
     * If the user is an admin or a manager, it calls the next middleware function. Otherwise, it returns a 401 error response.
     */
